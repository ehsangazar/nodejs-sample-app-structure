...
describe('AdsModel', () => {
  ...
  it('should get an error to create a repeated Entry', () => {
    const AdsObject = new AdsModel()
    AdsObject.create({name:'TestRepeated', standingTime: 'short', logo: 0, priority: 0})
    const resSecond = AdsObject.create({name:'TestRepeated', standingTime: 'short', logo: 0, priority: 0})
  ...
  it('should get an error if name is longer than 20 characters', () => {
    const AdsObject = new AdsModel()
    const res = AdsObject.create({name:'ABCDEFGHIJKLMNOPLMOJK', standingTime: 'short', logo: 0, priority: 0})
    expect(res.status).to.equal('error')
    expect(res.error).to.equal('name is not valid')
  });
  it('should get an error if standingTime is not valid', () => {
    const AdsObject = new AdsModel()
    const res = AdsObject.create({name:'validName', standingTime: 'notValid', logo: 0, priority: 0})
    expect(res.status).to.equal('error')
    expect(res.error).to.equal('standingTime is not valid')
  });
  it('should get an error if logo is not valid', () => {
    const AdsObject = new AdsModel()
    const res = AdsObject.create({name:'validName', standingTime: 'short', logo: 2, priority: 0})
    expect(res.status).to.equal('error')
    expect(res.error).to.equal('logo is not valid')
  });
  it('should get an error if priority is not valid', () => {
    const AdsObject = new AdsModel()
    const res = AdsObject.create({name:'validName', standingTime: 'short', logo: 1, priority: 3})
    expect(res.status).to.equal('error')
    expect(res.error).to.equal('priority is not valid')
  });
  it('should work to find an entry', () => {
    const AdsObject = new AdsModel()
    AdsObject.create({name:'FindingTest', standingTime: 'short', logo: 1, priority: 1})
