...
describe('AdsModel', () => {
  ...
  it('should work because data is enough', () => {
    ...
    const res = AdsObject.create({
      name: 'Test', standingTime: 'short', logo: 0, priority: 0, price: 1,
    });
  ...
  it('should get an error to create a repeated Entry', () => {
    const AdsObject = new AdsModel();
    AdsObject.create({
      name: 'TestRepeated', standingTime: 'short', logo: 0, priority: 0, price: 1,
    });
    const res = AdsObject.create({
      name: 'TestRepeated', standingTime: 'short', logo: 0, priority: 0, price: 1,
    });
    expect(res.status).to.equal('error');
    expect(res.error).to.equal('name exists in the db');
  });
  ...
  it('should get an error if name is longer than 20 characters', () => {
    const AdsObject = new AdsModel();
    const res = AdsObject.create({
      name: 'ABCDEFGHIJKLMNOPLMOJK', standingTime: 'short', logo: 0, priority: 0, price: 1,
    });
    expect(res.status).to.equal('error');
  ...
  it('should get an error if standingTime is not valid', () => {
    const AdsObject = new AdsModel();
    const res = AdsObject.create({
      name: 'validName', standingTime: 'notValid', logo: 0, priority: 0, price: 1,
    });
    expect(res.status).to.equal('error');
  ...
  it('should get an error if logo is not valid', () => {
    const AdsObject = new AdsModel();
    const res = AdsObject.create({
      name: 'validName', standingTime: 'short', logo: 2, priority: 0, price: 1,
    });
    expect(res.status).to.equal('error');
  ...
  it('should get an error if priority is not valid', () => {
    const AdsObject = new AdsModel();
    const res = AdsObject.create({
      name: 'validName', standingTime: 'short', logo: 1, priority: 3, price: 1,
    });
    expect(res.status).to.equal('error');
  ...
  it('should work to find an entry', () => {
    const AdsObject = new AdsModel();
    AdsObject.create({
      name: 'FindingTest', standingTime: 'short', logo: 1, priority: 1, price: 1,
    });
    const res = AdsObject.find({ name: 'FindingTest' });
    expect(res.status).to.equal('ok');
    expect(res.ads.name).to.equal('FindingTest');
  });
  ...
  it('should get an error if price is not valid', () => {
    const AdsObject = new AdsModel();
    const res = AdsObject.create({
      name: 'validName', standingTime: 'short', logo: 1, priority: 1, price: -3,
    });
    expect(res.status).to.equal('error');
